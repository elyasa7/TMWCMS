...
from article.models import Article, ArticleComment, ArticleCategory, ArticleImage
...
def residence_ru(request, article_id):
    ...
    args['simbars'] = Article.objects.filter(article_category_id=args['article'].article_category_id).order_by('id')[1:6]
...
def spas_ru(request):
    ...
    args['simspas'] = Article.objects.filter(article_category_id=args['article'].article_category_id).order_by('id')[1:6]
    args['minigals'] = ArticleImage.objects.filter(article_image_article_id=args['article'].id).order_by('id')[:6]
